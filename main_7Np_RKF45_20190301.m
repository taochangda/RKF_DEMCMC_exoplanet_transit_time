...
% the method of integration is RKF45(Runge-Kutta-Fehlberg4(5) )
%Version 1.0
...
if (DE_phase==1)
    ...
    addAttachedFiles(pool,[file_path,'Nbody_C_7Np_RKF45.mexw64']);
    ...
    while(1)
        ...
        parfor loop_1=1:n_link %  parallel compute. refer to matlab offical guide for more help 并行计算，需要调用大量分布式计算资源，计算快慢取决和计算资源和网络情况
            [tr,error_flag]=Nbody_C_7Np_RKF45(temp_para(loop_1,:),mc,Rs,tstop,time_precise,err);
...
conve_phase=0;
save([file_path,'FinalResult_RKF45_20190301.mat'],'-v7.3');
